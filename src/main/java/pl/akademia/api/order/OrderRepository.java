...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query(nativeQuery = true, value = "SELECT order_date, order_status, informations, delivery_method, payment_method FROM orders, client  where orders.client_id=client.id and orders.client_id= :id")
    List<Order> getOrderByClientId(Long id);

    @Query(nativeQuery = true, value = "SELECT order_date, order_status, informations, delivery_method," +
            " payment_method FROM orders, bike  where orders.basket_id=bike.id and bike.brand= :brand")
